...
<Window
  ...
  <Window.Resources>
    ...
    <!-- ConfirmBtn: green action button used for the Remove confirmation -->
...
# ── Build a section header TextBlock directly — no XAML parse ─────────────────
...
# ── Build a separator Border directly — no XAML parse ─────────────────────────
...
# ── Build a device row in WPF directly — no XAML parse per row ────────────────
# OPT: replaces ConvertFrom-XamlString per row, eliminating XML DOM + XamlReader
#      overhead for every single device in the list (biggest perf win for large scans).
function Add-DeviceRow {
    ...
    # Tag: raw InstanceId for ghosts; 'PROTECTED::' prefixed for protected devices
...
# ── Select All / None ─────────────────────────────────────────────────────────
# "Select All" intentionally skips protected devices — they must be opted-in manually
...
$script:BtnScan.Add_Click({
    ...
    [void]$script:ScanPS.AddScript({
        ...
        function Test-HasRealAffinity {
            ...
            try {
                ...
                if (Test-Path $afPath) {
                    ...
                    # DevicePolicy >= 3 means an explicit affinity assignment
        ...
        # Also check present devices for explicit affinity (protect them too)
    ...
    # Poll every 200 ms; animate the progress bar while waiting
    ...
    $script:PollTimer.Add_Tick({
        ...
        if ($script:ScanHandle -and $script:ScanHandle.IsCompleted) {
            ...
            # EndInvoke returns a collection; our PSCustomObject is at index 0
            ...
            $script:Win.Dispatcher.Invoke([action]{
                ...
                $total = $script:DevicesToRemove.Count + $script:DevicesProtected.Count
                ...
                if ($total -eq 0) {
                    ...
                    # FIX: enable Remove even when only protected devices are present,
                    # so users can manually check and force-remove them.
...
$script:BtnRemove.Add_Click({
    ...
    # Collect checked protected devices (user explicitly opted in by checking the box)
    ...
    # Count protected devices in the selection to decide whether to show the warning
    ...
    foreach ($device in $selected) {
        ...
        try {
            Get-PnpDevice -InstanceId $instanceId -ErrorAction Stop |
                Remove-PnpDevice -Confirm:$false -ErrorAction Stop
            $removed = $true
        } catch {}
        ...
        # Method 3: SetupAPI DiSetupDiRemoveDevice (pre-compiled at startup — no delay)
        if (-not $removed) {
            try {
        ...
        # Method 4: reg.exe delete — brute-force registry removal
        if (-not $removed) {
            try {
                & reg.exe delete "HKLM\SYSTEM\CurrentControlSet\Enum\$instanceId" /f 2>&1 | Out-Null
                if ($LASTEXITCODE -eq 0) { $removed = $true }
            } catch {}
        }

        # Method 5: devcon.exe remove (if present on PATH or beside the script)
        if (-not $removed) {
            try {
    ...
    # Delay rescan by 2 s so the Windows PnP manager has time to settle device states
